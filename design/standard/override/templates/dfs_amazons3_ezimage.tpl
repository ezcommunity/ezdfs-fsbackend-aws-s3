...
{if $image_content.is_valid}
    ...
    {if and( is_set( $image ), $image )}
        ...
        <img src="http://s3-{ezini( 'BackendSettings', 'Region', 'dfsamazons3.ini' )}.amazonaws.com/{ezini( 'BackendSettings', 'Bucket', 'dfsamazons3.ini' )}/{$image.url}" width="{$image.width}" height="{$image.height}" {if $hspace}hspace="{$hspace}"{/if} style="{$inline_style}" alt="{$alt_text|wash(xhtml)}" title="{$title|wash(xhtml)}" />
        {if $href}</a>{/if}
    {/if}
